...
def past_data(request):
    ...
    user_cases = HistoricalData.objects.all().filter(
        case_of=request.user.id).order_by('case_reg_date')
...
def test_patient(request):
    if request.method == 'POST':
        ...
        predicted_disease = diseases[pr1]+'(Pred. 1)'+' '+diseases[pr1]+'(Pred. 2)'+' '+diseases[pr1]+'(Pred. 3)'
        # dummy logic
